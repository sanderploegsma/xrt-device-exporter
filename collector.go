...
func (c *collector) Collect(ch chan<- prometheus.Metric) {
	for _, info := range c.repository.GetDeviceInfo() {
		...
		shell := info.Platforms[0].StaticRegion.VBNV

		for _, t := range info.Thermals {
			if t.IsPresent {
				ch <- prometheus.MustNewConstMetric(c.temperature, prometheus.GaugeValue, t.TempC, info.DeviceID, serial, shell, t.LocationID, t.Description)
			}
		}

		for _, p := range info.Electrical.PowerRails {
			if p.Voltage.IsPresent {
				ch <- prometheus.MustNewConstMetric(c.voltage, prometheus.GaugeValue, p.Voltage.Volts, info.DeviceID, serial, shell, p.Id, p.Description)
			}

			if p.Current.IsPresent {
				ch <- prometheus.MustNewConstMetric(c.current, prometheus.GaugeValue, p.Current.Amps, info.DeviceID, serial, shell, p.Id, p.Description)
			}
		}

		ch <- prometheus.MustNewConstMetric(c.powerConsumption, prometheus.GaugeValue, info.Electrical.PowerConsumptionWatts, info.DeviceID, serial, shell)
		ch <- prometheus.MustNewConstMetric(c.powerConsumptionMax, prometheus.GaugeValue, info.Electrical.PowerConsumptionMaxWatts, info.DeviceID, serial, shell)
		ch <- prometheus.MustNewConstMetric(c.powerConsumptionWarning, prometheus.GaugeValue, powerConsumptionWarningValue(info), info.DeviceID, serial, shell)
	}
}
